...
### Added
- **Drag & Drop support**:
  - Drag assets or folders from the Project window onto the Favorites window to add them; drop them on a group header to add them straight into that group
  - Drag a favorite onto a group header to move it into that group, or onto the list background to ungroup it
  - Drag a favorite out of the window into the Scene view, Hierarchy, or Inspector object fields — it carries the real asset reference
  - Drop targets highlight while dragging over them

## [1.2.3] - 2026-07-05

### Changed
- First release distributed by OpenUPM in `githubRelease` mode, so the signed `.tgz` (containing `package/.attestation.p7m`) is served through the registry and Unity Package Manager can verify the package signature.

## [1.2.2] - 2026-07-05

### Changed
- No functional changes; validates the signed release pipeline end-to-end.

## [1.2.1] - 2026-07-04

### Added
- **Signed releases**: GitHub Actions workflow that packs and signs the package with the Unity UPM CLI on every `v*` tag and attaches the signed `.tgz` to the GitHub Release.

### Fixed
- Corrected the placeholder `repository.url` in `package.json` to point at the real repository.
